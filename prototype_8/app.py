...
def ask_question():
    ...
    try:
        ...
        raise Exception("Error establising connection to database")
    ...
    if re.search(url_pattern, message) and scrape_command in message.lower():
        ...
        filename = f"scrape_output_{timestamp}.json"
        ...
        try:
            # Run Scrapy command
            subprocess.run(['scrapy', 'crawl', 'gas', '-O', filepath, '-a', f'start_url={url}'], check=True)
            print(f"Scraping completed, output saved to {filepath}")  # Debug log

            # Read the contents of the scraped file
            with open(filepath, 'r') as f:
                scraped_data = json.load(f)

            # Create a response message with download link
            ...
            for i, item in enumerate(scraped_data[:5], 1):  # Show first 5 items
                response_text += f"\n{i}. {json.dumps(item, indent=2)}"
            
            if len(scraped_data) > 5:
                response_text += f"\n\n... and {len(scraped_data) - 5} more items."
            
            response_text += f"\n\nYou can download the complete data here: [Download Scraped Data]({download_url})"
