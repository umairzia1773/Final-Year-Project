...
def signup():
    if request.method == 'POST':
        ...
        print("Received CAPTCHA Token:", recaptcha_response)
        ...
        if not is_password_strong(password):
            print("Password validation result:", is_password_strong(password)) 

            return jsonify(success=False, message="Password must be at least 8 characters long and include: 1 uppercase letter, 1 lowercase letter, 1 number, and 1 special character.")
...
def password():
    if request.method == "POST":
        ...
        if user:
            ...
            print("email not found in data base")
...
def submit():
    ...
    secret_key = '6LdhBOcqAAAAAF3yO9Oex4XsQsWi-gVuHMM_uYk7'
    ...
    if result.get('success'):
        ...
        return jsonify(success=False, error="CAPTCHA validation failed")
@app.route('/chat', methods=["GET", "POST"])
def chat():
    user_id = session.get('user_id')
    print(f"[DEBUG] ✅ Flask user_id before rendering chat: {user_id}")
    # print(f"[DEBUG] Chat Route - Session User ID: {session.get('user 
    # _id')}")
