...
def ask_question():
    ...
    try:
        ...
        if re.search(url_pattern, message) and scrape_command in message.lower():
            ...
            # Check if format is specified in the message
            specified_format = None
            valid_formats = {'json', 'csv', 'xml'}
            
            # Check for format in the message
            message_lower = message.lower()
            for fmt in valid_formats:
                if f'.{fmt}' in message_lower or f'{fmt} format' in message_lower or f'{fmt} file' in message_lower:
                    specified_format = fmt
                    break

            # Generate a unique filename base based on timestamp
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filepath_base = os.path.join('scraped_data', f"scrape_output_{timestamp}")

            # Ensure the scraped_data directory exists
            os.makedirs('scraped_data', exist_ok=True)

            try:
                # First save as JSON (default format for scrapy)
                temp_json_file = f"{filepath_base}_temp.json"
                subprocess.run(['scrapy', 'crawl', 'gas', '-O', temp_json_file, '-a', f'start_url={url}'], check=True)
                ...
                if specified_format:
                    # User specified a format - directly provide that format
                    output_file = convert_to_format(scraped_data, specified_format, filepath_base)
                    download_url = url_for('download_file', filename=os.path.basename(output_file), _external=True)
                    
                    response_text = f"Scraping completed successfully! Found {len(scraped_data)} items.\n\n"
                    response_text += f"Here's a summary of the scraped data:\n"
                    for i, item in enumerate(scraped_data[:5], 1):
                        response_text += f"\n{i}. {json.dumps(item, indent=2)}"
                    if len(scraped_data) > 5:
                        response_text += f"\n\n... and {len(scraped_data) - 5} more items."
                    response_text += f"\n\nDownload the complete data in {specified_format.upper()} format: [Click to Download]({download_url})"

                else:
                    # No format specified, create all formats and provide options
                    format_files = {}
                    for fmt in valid_formats:
                        output_file = convert_to_format(scraped_data, fmt, filepath_base)
                        format_files[fmt] = url_for('download_file', filename=os.path.basename(output_file), _external=True)
                    ...
                    for fmt, url in format_files.items():
                        response_text += f"\n- [{fmt.upper()} format]({url})"
...
def convert_to_format(data, format_type, filepath_without_ext):
    """Convert scraped data to specified format and save."""
    import json
    import csv
    import dicttoxml
    ...
    if format_type == 'json':
        output_file = f"{filepath_without_ext}.json"
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(data, f, indent=2)
        return output_file
    
    elif format_type == 'csv':
        output_file = f"{filepath_without_ext}.csv"
        ...
        output_file = f"{filepath_without_ext}.xml"
